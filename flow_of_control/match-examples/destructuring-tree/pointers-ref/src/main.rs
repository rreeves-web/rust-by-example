// Differences between dereferencing pointers and destructuring
// them:
// Dereferencing uses *
...
fn main() {
    ...
    match reference {
        // If `reference` is a pattern matched against `&val`,
        // it results in a comparison like:
        // `&i32`
        // `&val`
        // ^ We see that if the matching `&`s are dropped, then
        // the `i32` should be assigned to `val`.
    ...
    match mut_value {
        ref mut m => {
            // Got a reference. Gotta dereference it before we can add
            // anything to it.
            *m += 10;
