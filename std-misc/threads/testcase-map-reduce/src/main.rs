...
// automatically prevent you from manipulating a state that is visible to other
...
// understands that we're only psasing read-only referneces, and that thus no
...
fn main() {
    ...
    ************************************************************************/
    ...
    for (i, data_segment) in chunked_data.enumerate() {
        ...
        // 'move || -> u32' is the syntax for a closure that:
        // * takes no arguments ('||')
    ...
    ************************************************************************/
    ...
    // we use the "turbofish" ::<> to privde sum() with a type hint.
...
// It is not wise to let our number of threads depend on user inputted ata. What if
// the user decides to insert a lot of spaces? Do we *really* want to spawn 2,000
// threads? Modify the program so that the data is always chunked into a limited
// number of chunks, defined by a static constant at the beginning of the
// program.
...
// - unwrap vs. expect
// - enumerate
// Links to all of the above at the bottom of:
