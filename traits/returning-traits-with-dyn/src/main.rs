...
// function that returns `Animal`, because its different implementations will need
// different amounts of memory.

// However, there's an easy workaround. Instead of returning a trait object directly,
// our functions return a `Box` which *contains* some `Animal`. A `box` is just a
// reference to some memory in the heap. Because a reference has a statically-known
// size, and the compiler can guarantee it points to a heap-allocated `Animal`,
