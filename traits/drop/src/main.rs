// The `Drop`[url]https://doc.rust-lang.org/std/ops/trait.Drop.html[/url] trait
// only has one method: `drop`, which is called automatically when an object
// goes out of scope. The main use of the `Drop` trait is to free the resources
...
fn main() {
    ...
    // `_a` *won't* be `drop`ed again here, because it has already
    // (manually) `drop`ed
}
