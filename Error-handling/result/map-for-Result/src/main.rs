...
// the `Err` type, we look to `parse()`, which is implemented with the `FromStr`
// trait for `i32`. As a result, the `Err` type is specified as `ParseIntError`.
...
// With the return type rewritten, we use pattern matching without `unrap()`.
