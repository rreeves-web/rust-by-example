...
// are many combinators beween `Result` and `Option` that overlap.

// In working with Rust, you will encounter methods that return the `Result` type,
// such as the `parse()` method. It might not always be possible to parse a string
// into another type, so `parse()` returns a `Result` indicating possible failure.

// Let's see what happens when we successfully and unsuccessfully `parse()` a
// string:
fn multiply(first_number_str: &str, second_number_str: &str) -> i32 {
    // Let's first try using `unwrap()` to get the number out. will it bite us?
