...
// compilation fail since `main` returns `unit`. The ability to hide some source
// lines from documentation comes to the rescue: one may write `fn try_main ->
// Result<(), ErrorType>`, hide it and `unwrap` it in hidden `main`. Sounds
// complicated? Here's an example:
// ```Link to Guidelines snippet```:https://rust-lang-nursery.github.io/api-guidelines/documentation.html#examples-use--not-try-not-unwrap-c-question-mark

/// Using hidden `try_main` in doc tests.
///
/// ```
/// # // hidden lines start with a `#` symbol, but they're still compilable!
